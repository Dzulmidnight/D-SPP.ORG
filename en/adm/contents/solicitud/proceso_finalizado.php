...
if(isset($_POST['comprobanteMembresia']) && $_POST['comprobanteMembresia'] == "2"){
  ...
  if(isset($_POST['aprobar'])){
    ...
    ';
        ...
        $headers .= "Bcc: yasser.midnight@gmail.org\r\n";
        //$headers .= "Bcc: isc.jesusmartinez@gmail.org  \r\n"; 

        mail($destinatario,$asunto,utf8_decode($cuerpo),$headers);

        $queryMensaje = "INSERT INTO mensajes(idopp, asunto, mensaje, destinatario, remitente, fecha) VALUES($idopp, '$asunto', '$cuerpo', 'OPP', 'ADM', $fecha_actual)";
...
////////////////////////////////////////////////////////////////////////////////
        ...
        while($emailADM = mysql_fetch_assoc($ejecutar)){  
          if($emailADM['email'] != "isc.jesusmartinez@gmail.org"){
...
if(isset($_POST['resolucionObjecion']) && $_POST['resolucionObjecion'] == "resolucionObjecion"){
  ...
  $row_oc = mysql_fetch_assoc($ejecutar);
    ...
    ';
        ...
        $mail->AddBCC("yasser.midnight@gmail.org", "correo Oculto");
...
/*******************************************************************************************/
 ...
 if(isset($_POST['filtroPalabra']) && $_POST['filtroPalabra'] == "1" && $_POST['palabraClave']){
  $palabraClave = $_POST['palabraClave'];
        ...
        $query_buscar = "SELECT solicitud_certificacion.*, opp.idopp,opp.nombre, opp.pais, opp.estado, opp.abreviacion AS 'abreviacionOPP', oc.idoc, oc.nombre AS 'nombreOC', oc.abreviacion FROM solicitud_certificacion INNER JOIN opp ON solicitud_certificacion.idopp = opp.idopp INNER JOIN oc ON solicitud_certificacion.idoc = oc.idoc WHERE (solicitud_certificacion.status = 10) AND (opp.nombre LIKE '%$palabraClave%') OR (solicitud_certificacion.p1_email LIKE '%$palabraClave%') OR (opp.pais LIKE '%$palabraClave%') OR (oc.nombre LIKE '%$palabraClave%') OR (oc.abreviacion LIKE '%$palabraClave%') ORDER BY solicitud_certificacion.fecha_elaboracion DESC";

        $queryTotal = "SELECT COUNT(idsolicitud_certificacion) AS 'totalConsulta' FROM solicitud_certificacion INNER JOIN opp ON solicitud_certificacion.idopp = opp.idopp INNER JOIN oc ON solicitud_certificacion.idoc = oc.idoc WHERE (solicitud_certificacion.status = 10) AND (opp.nombre LIKE '%$palabraClave%') OR (solicitud_certificacion.p1_email LIKE '%$palabraClave%') OR (opp.pais LIKE '%$palabraClave%') OR (oc.nombre LIKE '%$palabraClave%') OR (oc.abreviacion LIKE '%$palabraClave%') ORDER BY solicitud_certificacion.fecha_elaboracion DESC";

}else if(!isset($_POST['filtroPalabra']) || empty($_POST['palabraClave'])){
        $query_buscar = "SELECT solicitud_certificacion.*, opp.idopp,opp.nombre, opp.pais, opp.estado,opp.abreviacion AS 'abreviacionOPP', oc.idoc, oc.nombre AS 'nombreOC', oc.abreviacion FROM solicitud_certificacion INNER JOIN opp ON solicitud_certificacion.idopp = opp.idopp INNER JOIN oc ON solicitud_certificacion.idoc = oc.idoc WHERE (solicitud_certificacion.status = 10) ORDER BY solicitud_certificacion.fecha_elaboracion DESC";
        $queryTotal = "SELECT COUNT(idsolicitud_certificacion) AS 'totalSolicitudes' FROM solicitud_certificacion WHERE status = 10";
...
<hr>
  ...
  <!----------------------------------------- INICIA COTIZACION OPP ---------------------------------------------------------->
          <td>  
            <a href="http://d-spp.org/oc/<?echo $registro_busqueda['cotizacion_opp']?>" target="_blank" type="button" class="botonCotizacion btn <?php if(empty($registro_busqueda['cotizacion_opp'])){ echo 'btn-default btn-sm';}else{echo 'btn-success btn-sm';} ?>" aria-label="Left Align" <?php if(empty($registro_busqueda['cotizacion_opp'])){echo "disabled";}?>>
                            ...
                            <div class="col-xs-6">
                              ...
                              <?php }else{ ?>
                                <div class="col-xs-12 alert alert-success" role="alert">
                                  <div class="col-xs-12">Estatus Certificado al día: <b><?echo date("Y/m/d", $registroFecha['ultimaFecha']) ?></b></div>
                                  <hr>
                                  <div class="col-xs-12 ">
                                    <?
                                    ...
                                    <hr>
                                    Su certificado vence el dia: <?echo $certificado['vigenciafin']?>
