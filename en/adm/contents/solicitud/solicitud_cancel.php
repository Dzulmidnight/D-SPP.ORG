...
/*******************************************************************************************/
 ...
 if(isset($_POST['filtroPalabra']) && $_POST['filtroPalabra'] == "1" && $_POST['palabraClave']){
  $palabraClave = $_POST['palabraClave'];
        ...
        $query_buscar = "SELECT solicitud_certificacion.*, opp.idopp,opp.nombre, opp.pais, opp.estado,opp.abreviacion AS 'abreviacionOPP', oc.idoc, oc.nombre AS 'nombreOC', oc.abreviacion FROM solicitud_certificacion INNER JOIN opp ON solicitud_certificacion.idopp = opp.idopp INNER JOIN oc ON solicitud_certificacion.idoc = oc.idoc WHERE (solicitud_certificacion.status = 14 OR solicitud_certificacion.status = 24 ) ORDER BY solicitud_certificacion.fecha_elaboracion DESC";
        $queryTotal = "SELECT COUNT(idsolicitud_certificacion) AS 'totalSolicitudes' FROM solicitud_certificacion WHERE (status = 14 OR status = 24)";
...
<hr>
 ...
 <!-----------------------------------    INICIA SECCION INFORMACIÓN ESTATUS ---------------------------------------------->
          <td class="text-center">
            ...
            if(empty($total)){ 
              ?>
              <button <?if(isset($row_objecion['idobjecion'])){echo "class='botonObjecion btn btn-success btn-sm'";}else{echo "class='btn btn-danger btn-sm'";}if($registro_busqueda['status'] == 24){echo "disabled";}?> data-toggle="modal" <?php echo "data-target='#myModal".$registro_busqueda['idsolicitud_certificacion']."'"?> >
