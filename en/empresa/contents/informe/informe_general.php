...
if(isset($_POST['crear_informe'])){
	if($_POST['crear_informe'] == 'YES'){
		...
		echo "<script>alert('The $idinforme_general report for the year $ano has been created.');</script>";
	}else{
...
if(isset($_POST['informe_trimestral'])){
	if($_POST['informe_trimestral'] == 'YES'){
		...
		$insertSQL = sprintf("INSERT INTO trim1 (idtrim1, idempresa, fecha_inicio, fecha_fin, total_trim1, estado_trim1, valor_contrato_trim1, cuota_uso_trim1, estatus_factura_trim1, estatus_comprobante_trim1) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
		...
		echo "<script>alert('A new quarterly format has been created');</script>";
	}else{
...
<div class="row">
	...
	if($informe_general['idinforme_general']){
		...
		?>
			<form action="" method="POST">
				...
				Step 2: No <b style="color:red">"Quarterly Format"</b> has been started in the <b style="color:red">"General Report <?php echo $ano_actual ?>"</b>, <strong>Do you want to create a new Format for Quarterly Report?</strong>
	...
	?>		
		<form action="" method="POST">
			...
			Step 1: No General Reports Found, <strong>Would you like to create a new General Report?</strong>
