...
mysql_select_db($database_dspp, $dspp);
$query_accion_detalle = sprintf("SELECT solicitud_certificacion.* FROM solicitud_certificacion WHERE idsolicitud_certificacion = %s", GetSQLValueString($colname_accion_detalle, "int"));
...
<? }?>
  ...
  <?php if($row_solicitud['status'] == "DENEGADO"){ ?>
    <div class="panel panel-danger">
      <div class="panel-heading">
        <h3 class="panel-title">Razones por las que se denego</h3>
      </div>
      <div class="panel-body">
        <?php if(isset($row_solicitud)) ?>
        <?php echo $row_solicitud['observaciones']; ?>
      </div>
    </div>
  <?php } ?>

<form class="" method="post" name="form1" action="pruebas2.php" enctype="multipart/form-data">
	...
	<table class="table table-bordered table-striped col-xs-8">
		...
		<tr>
			...
			<td colspan="2">
				<input type="text" autofocus="autofocus" class="form-control" id="exampleInputEmail1" size="70" placeholder="Nombre Organización" value="<?php echo $_SESSION['nombre']?>" disabled>
			</td>
		</tr>
		<tr>
			<td colspan="2">RFC</td>
...
$texto = $row_solicitud['op_resp4'];
 ?>
    ...
    <tr>
      <td colspan="2">
        Nombre y firma del personal del OC, que recibe la solicitud:
      </td>
      <td colspan="2">
        <input type="text" class="form-control" name="personal_oc" disabled>
      </td>
    </tr> 
  ...
  <input type="hidden" name="idsolicitud_certificacion" value="<?php echo $_GET['idsolicitud']?>">
	<input class="btn btn-primary" type="submit" value="Actualizar Solicitud">
</form>
