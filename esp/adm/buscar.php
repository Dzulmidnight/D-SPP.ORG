...
if(isset($_POST['consulta'])){
	$q = $_POST['consulta'];
	$query = "SELECT contactos.*, opp.abreviacion AS 'abreviacion_opp', opp.pais FROM contactos INNER JOIN opp ON contactos.idopp = opp.idopp WHERE contactos.nombre LIKE '%".$q."%' OR opp.pais LIKE '%".$q."%' GROUP BY contactos.nombre ORDER BY contactos.nombre ASC";
}
...
if($total > 0){
	...
	while ($fila = mysql_fetch_assoc($resultado)) {
		...
		"<tr>
			...
			<td>".'id oc: '.$fila['idoc']."</td>
			<td>".'lista: '.$fila['lista_contactos']."</td>
			<!-- ABREVIACIÓN ORGANIZACIÓN -->
			<td>".$fila['idopp'].' - <a href="?OPP&detail&idopp='.$fila['idopp'].'">'.mayuscula($fila['abreviacion_opp']).'</a>'."</td>
			<!-- PAIS -->
