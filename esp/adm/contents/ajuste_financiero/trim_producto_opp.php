...
if(isset($_POST['agregar_formato']) && $_POST['agregar_formato'] == 1){
	...
	$insertSQL = sprintf("INSERT INTO trim_productos_opp(producto_general, tipo_producto, volumen_unitario, volumen_total, ventas_totales, tipo_moneda) VALUES(%s, %s, %s, %s, %s, %s)",
		...
		GetSQLValueString($volumen_total, "text"),
		...
		<form class="form-horizontal" method="POST">
		 	<table class="table table-bordered table-condensed" style="font-size:11px;" id="tablaInforme">
		 		...
		 		<tbody>
		 			...
		 			<?php 
		 			$row_trim_opp = mysql_query("SELECT * FROM trim_productos_opp", $dspp) or die(mysql_error());
		 			$num_trim_opp = mysql_num_rows($row_trim_opp);
		 			if($num_trim_opp > 0){
		 				$contador = 1;
		 				while($trim_opp = mysql_fetch_assoc($row_trim_opp)){
		 				?>
		 				<tr>
		 					...
		 					<td><?php echo $trim_opp['volumen_total']; ?></td>
		 					<td><?php echo $trim_opp['ventas_totales']; ?></td>
		 					<td><?php echo $trim_opp['tipo_moneda']; ?></td>
		 				</tr>
		 				<?php
		 				$contador++;
		 				}
		 			}else{
		 				echo "<tr class='warning'><td colspan='7'>Registros no encontrados</td></tr>";
		 			}
		 			 ?>
					...
					<tr class="success">
						...
						<td>
							<input type="text" class="form-control" name="tipo_producto" id="" placeholder="tipo_producto" required>
						</td>
						<td>
							<input type="text" class="form-control" name="volumen_unitario" placeholder="peso o volumen unitario">
						</td>
						<td>
							<input type="text" class="form-control" name="volumen_total" placeholder="volumen_total">
						</td>
						<td>
							<input type="number" step="any" class="form-control" name="ventas_totales" placeholder="valor_ventas_totales" required>
						</td>
						<td>
