...
function preguntar(){ 
    if(!confirm('¿Estas seguro de eliminar el registro?, los datos se eliminaran permanentemente')){ 
       return false; } 
} 
...
/// DEBEN DE TENER UN ESTATUS-DSPP DEL 1 al 9, o el 17
  $query = "SELECT opp.idopp, opp.spp, opp.abreviacion, opp.pais, solicitud_certificacion.tipo_solicitud, solicitud_certificacion.estatus_dspp FROM solicitud_certificacion INNER JOIN opp ON solicitud_certificacion.idopp = opp.idopp WHERE (solicitud_certificacion.estatus_dspp BETWEEN 1 AND 9 OR solicitud_certificacion.estatus_dspp = 17) AND solicitud_certificacion.tipo_solicitud = 'NUEVA'";
...
<table class="table table-bordered">
  ...
  <?php
  $contador = 1;
  while($detalle_opp = mysql_fetch_assoc($consultar)){
    echo '<tr>';
      echo '<td>'.$contador.'</td>';
      echo '<td>'.$detalle_opp['idopp'].'</td>';
      echo '<td>'.$detalle_opp['spp'].'</td>';
      echo '<td>'.$detalle_opp['abreviacion'].'</td>';
    echo '</tr>';
    $contador++;
  }
   ?>
