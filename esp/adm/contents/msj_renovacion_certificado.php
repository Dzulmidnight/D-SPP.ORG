    ...
    if($time_vencimiento >= $time_actual){ /// inicia IF 1
      ...
      if($time_restante <= $plazo){ /// inicia IF 2
        ...
        if(!isset($aviso_renovacion['idaviso_renovacion'])){ // inicia IF 3
          ...
          // primer aviso de renovación del certificado (2 meses antes)
          $cuerpo = '
            <html>
              <head>
                <meta charset="utf-8">
          ...
          $estatus_certificado = 10; // CERTITICADO ACTIVO
        } // termina else 3
      }/// termina IF 2
      else{ /// inicia ELSE 2
        if($prorroga >= $time_actual){ // inicia if 1.1
          $estatus_certificado = 15; // CERTIFICADO POR EXPIRAR (estatus_dspp)
          if(!isset($aviso_renovacion['aviso2'])){ // inicia if 1.1.1
            if(!isset($aviso_renovacion['idaviso_renovacion'])){ // inicia if 1.1.2
            ...
            $asunto = "2do D-SPP - Certificado por expirar / Certified for expiring";
