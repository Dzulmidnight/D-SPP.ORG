<?php 
  ...
  while($certificado = mysql_fetch_assoc($row_certificado)){ /// INICIA WHILE 1
    ...
    /*19_09_2017 if($plazo <= $primero){ /// se valida el envio de los primeros 2 aviso
      if(($plazo > $segundo && $plazo <= $primero) && !isset($aviso_renovacion['idaviso_renovacion'])){ // se valida el envio del PRIMER AVISO
        $asunto = $aviso_renovacion['idaviso_renovacion']."1er Aviso de Renovación del Certificado / 1st Certificate Renewal Notice";
        ...
        $insertSQL = sprintf("INSERT INTO avisos_renovacion(idopp, aviso1, ano_aviso, idcertificado, fecha_certificado) VALUES(%s, %s, %s, %s, %s)",
            ...
            GetSQLValueString($certificado['vigencia_fin'], "text"));
        $insertar = mysql_query($insertSQL, $dspp) or die(mysql_error());
      }else if($segundo >= $plazo && !isset($aviso_renovacion['aviso2'])){ // se valida el envio del SEGUNDO AVISO
        $asunto = $aviso_renovacion['idaviso_renovacion']."2do Aviso de Renovación del Certificado / 2nd Certificate Renewal Notice";
    ...
    }else if($time_vencimiento >= $time_actual && !isset($aviso_renovacion['aviso3'])){ /// se valida el envio del 3º aviso
      $asunto = $aviso_renovacion['idaviso_renovacion']."-3er Aviso de Renovación del Certificado - Alerta de suspensión / 3rd Certificate Renewal Notice - Suspension alert";
          ...
          <body>
            <table style="font-family: Tahoma, Geneva, sans-serif; font-size: 13px; color: #797979;" border="0" width="650px">
              <tbody>
                ...
                <tr>
                  <td style="text-align:justify; padding-top:2em" colspan="2">
                    <p>Estimados Representantes de <strong style="color:red">'.$aviso_renovacion['idaviso_renovacion'].', (<u>'.$abreviacion_opp.'</u>)</strong>:</p>
      ...
      $asunto = $aviso_renovacion['idaviso_renovacion']."Suspensión del certificado / Suspension of certificate";
